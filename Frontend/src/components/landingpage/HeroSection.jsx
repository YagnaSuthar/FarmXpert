...
export default function HeroSection() {
  useEffect(() => {
    ...
    function drawBranch(b, progress) {
      ...
      g.addColorStop(0,   `rgba(215,210,200,${Math.min(1, a * 1.8)})`);
      g.addColorStop(0.3, `rgba(185,180,172,${a * 1.3})`);
      g.addColorStop(0.7, `rgba(145,141,134,${a * 0.8})`);
      g.addColorStop(1,   `rgba(80,78,73,${a * 0.28})`);
      ctx.beginPath();
      ...
      if (d === 0 && progress > 0.5) {
        ...
        ctx.moveTo(sp[0].x, sp[0].y);
        for (let i = 1; i <= endIdx; i++) ctx.lineTo(sp[i].x, sp[i].y);
        ctx.strokeStyle = `rgba(200,196,186,${0.04 * Math.min(1, progress)})`;
    ...
    function drawPulses(ts) {
      ...
      pulses.forEach((p) => {
        ...
        ctx.fillStyle = `rgba(220,215,205,${0.88 * fade})`; ctx.fill();
        ctx.beginPath(); ctx.arc(pt.x, pt.y, 5, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(220,215,205,${0.12 * fade})`; ctx.fill();
      });
    }
    ...
    function drawSoil(alpha) {
      ...
      soilPts.forEach(([nx, ny]) => {
        ...
        ctx.fillStyle = `rgba(160,155,148,${alpha * (0.12 + d * 0.72)})`;
        ctx.fill();
      });
    }
    ...
    function drawSeed(ts) {
      ...
      ctx.fillStyle = `rgba(200,196,186,${0.04 + pulse * 0.06})`; ctx.fill();
    }
