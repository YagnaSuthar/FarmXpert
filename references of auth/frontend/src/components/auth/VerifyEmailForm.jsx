  ...
  const handleResend = async () => {
    if (countdown === 0 && email.trim()) {
      ...
      try {
        ...
        setInfoMessage(res.message || 'Verification code resent');
        setCountdown(59);
        setDigits(['', '', '', '', '', '']);
  ...
  return (
    <>
      <header className="form-head-auth">
        <h1 className="form-title-auth">{t('verifyEmail.title')}</h1>
        <p className="form-sub-auth">
          {email ? (
            <>
              {t('verifyEmail.subtitle')} (<strong>{email}</strong>)
            </>
          ) : (
      ...
      {isSuccess ? (
        <div className="login-form-auth">
          <div className="status-banner-auth">
            ...
            <div>
              <h3 className="status-title-auth">{t('verifyEmail.title')}</h3>
              <p className="status-desc-auth">{t('verifyEmail.verifiedSuccess')}</p>
            </div>
          </div>
        </div>
      ) : (
        <form className="login-form-auth" onSubmit={handleSubmit} noValidate>
          ...
          {!initialEmail && (
            <div className="field-auth">
              <label className="field-label-auth" htmlFor="verify-email-input">
          ...
          <div className="field-auth">
            <label className="field-label-auth">{t('verifyEmail.codeLabel')}</label>
          ...
          >
            <span>
              {isSubmitting
                ? t('verifyEmail.submittingButton')
